...
async function importData() {
  ...
  try {
    // Parse CSV
    const employees = parseCSV(csvFilePath)
    console.log(`✓ Parsed ${employees.length} employees from CSV`)

    // Take first 50 employees for testing (you can remove this limit later)
    const employeesToImport = employees.slice(0, 50)
    console.log(`✓ Importing first ${employeesToImport.length} employees...`)
    ...
    if (allEmployees) {
      for (const emp of allEmployees) {
        try {
          await supabase.rpc('calculate_competency_gap', {
            employee_id: emp.EmployeeNumber
          })

          await supabase.rpc('check_promotion_readiness', {
            employee_id: emp.EmployeeNumber
          })

          console.log(`  ✓ Processed Employee ${emp.EmployeeNumber}`)
        } catch (err) {
          console.log(`  ✗ Error processing Employee ${emp.EmployeeNumber}`)
        }
      }
    }
